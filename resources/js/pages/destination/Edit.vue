...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    },
    {
        ...
        href: `/destination/${props.destination.id}/edit`,
    },
];
...
const form = useForm({
    ...
    lon: props.destination.lon,
    pic_id: String(props.destination.pic_id),
    published: props.destination.published,
    categories: props.destination.categories?.map(category => category.id) || [] as (string | number)[],
...
function handleSubmit() {
    form.post(`/destination/${props.destination.id}`, {
        onSuccess: () => {
            toast.success('Destinasi berhasil diperbarui', {
                description: 'Data destinasi telah diperbarui dengan sukses',
            });
        },
        onError: () => {
            toast.error('Gagal memperbarui destinasi', {
                description: 'Terjadi kesalahan saat memperbarui data destinasi',
            });
        },
    });
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <Card class="m-4">
            <form @submit.prevent="handleSubmit">
                <CardHeader>
                    <CardTitle class="mb-2">Edit Destinasi</CardTitle>
                    <CardDescription>Perbarui informasi destinasi wisata</CardDescription>
                </CardHeader>
